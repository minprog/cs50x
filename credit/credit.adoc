...
* In `credit.c` in `~/problems/credit/`, write a program that prompts the user for a credit card number and then reports (via `printf`) whether it is a valid American Express, MasterCard, or Visa card number, per the definitions of each's format herein.
...
image::walkthrough.jpg[link="https://youtu.be/06wNhN54P6k"]
